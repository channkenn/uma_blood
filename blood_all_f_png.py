...
def create_combined_bloodline_image(names, bloodlines_dict):
    ...
    dot.attr(ranksep="3.0", nodesep="0.1")  # ノード間の間隔を広げる
    dot.attr(bgcolor='transparent')  # 背景を透過に設定
    dot.attr(dpi="40")  # DPIを150に設定（通常は96）
    ...
    def process_individual(name, depth=0):
        ...
        if child != "unknown_child":
            dot.node(child, shape="box", image=get_image_path(unidecode(child)), width="0.1", height="0.1", label=child, fontname="MS Gothic")
